...
max_control_constraint = @(t, x, u, p) u(1) - u_max;
min_control_constraint = @(t, x, u, p) 0 - u(1);
control_convex_constraints = {max_control_constraint, min_control_constraint};
...
%% Specify Objective
min_fuel_objective = @(x, u, p) sum(u(1, :)) * tf / (N - 1);
...
u = ptr_sol.u(1, :, ptr_sol.converged_i + 1) .* [cos(ptr_sol.u(2, :, ptr_sol.converged_i + 1)); sin(ptr_sol.u(2, :, ptr_sol.converged_i + 1))];
...
function [xdot] = state_equation(x, u, mu, alpha)
    ...
    u_vec = u(1) * [cos(u(2)); sin(u(2))];

    xdot = [f0 + B * u_vec / m; m_dot];
end
