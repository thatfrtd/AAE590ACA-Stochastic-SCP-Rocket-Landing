...
%% Get Dynamics
f = @(t, x, u, p) p(1) * SymDynamicsQuat6DoF_localrot_noumag(x, u, L, I, alpha, g);
...
time_constraint = {1, @(t, x, u, p) 30 - p(1)};

state_convex_constraints = {mass_constraint, angular_velocity_constraint, time_constraint};
...
max_vane_angle_constraint = {1:N, @(t, x, u, p) abs(u(4)) - deg2rad(0.5)};
...
pitch_constraint = @(t, x, u, p) 1 - (2 * (x(7, :) .* x(9, :) - x(8, :) .* x(10, :))) .^ 2 - sin(deg2rad(60)) ^ 2;
...
elseif u_hold == "FOH"
    min_fuel_objective = @(x, u, p, x_ref, u_ref, p_ref) -x(14, end) / m_0;% + sum(abs(u(4, :))) + 0 * sum(abs(x(11, :)));% + sum_square(u(4, :)); %sum((u(4, 1:(end - 1)) + u(4, 2:end)) / 2) * delta_t;
end
...
prob_6DoF = DeterministicProblem(x_0, x_f, N, u_hold, 1, f, guess, convex_constraints, min_fuel_objective, scale = scale, scale_hint = scale_hint, terminal_bc = terminal_bc, nonconvex_constraints = nonconvex_constraints, discretization_method = "error", N_sub = 1);
