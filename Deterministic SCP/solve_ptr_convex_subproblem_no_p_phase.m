function [x_sol, u_sol, sol_info] = solve_ptr_convex_subproblem_no_p_phase(prob, ptr_ops, x_ref, u_ref)
        ...
        for k = 1:prob.Nu
            ...
            for cc = 1:prob.n.cvx
                ...
                if ismember(k, cc_k)
                    ...
                    cvx_constraint_func(prob.t_k(k), prob.unscale_x(X(:, k)), prob.unscale_u(U(:, k)), 0) <= 0;
                end
            end
            ...
            for nc = 1:prob.n.ncvx
                ...
                if ismember(k, nc_k)
                    ...
                    ncvx_constraint_func(prob.t_k(k), prob.unscale_x(X(:, k)), prob.unscale_u(U(:, k)), 0, prob.unscale_x(x_ref), prob.unscale_u(u_ref), 0, k) ...
                        - v_prime(nc) <= 0;
                end
            end
        end
        ...
        for e = 1:(prob.n.phase - 1)
            ...
            for pcc = 1:prob.phase_transition{e}.n.cvx
                ...
                if cvx_constraint_type == "<="
                    cvx_constraint_func(prob.t_k(ph_k), prob.unscale_x(X(:, [ph_k, ph_k + 1])), prob.unscale_u(U(:, [ph_k, ph_k + 1])), 0) <= 0;
            ...
            for pnc = 1:prob.phase_transition{e}.n.ncvx
                ...
                if ncvx_constraint_type == "<="
                    ncvx_constraint_func(prob.t_k(ph_k), prob.unscale_x(X(:, [ph_k, ph_k + 1])), prob.unscale_u(U(:, [ph_k, ph_k + 1])), 0, prob.unscale_x(x_ref(:, [ph_k, ph_k + 1])), prob.unscale_u(u_ref(:, [ph_k, ph_k + 1])), 0) ...
