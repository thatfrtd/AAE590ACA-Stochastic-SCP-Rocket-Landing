 ...
 *
 * PIPG_orig_precond_Dexplicit_emxutil.c
 *
...
{
  emxFree_real_T(sp, &pStruct->zhat_inf_dj);
  emxFree_real_T(sp, &pStruct->what_inf_dj);
  emxFree_real_T(sp, &pStruct->zhis);
}
...
{
  pStruct->Value.size[0] = 0;
  pStruct->Value.size[1] = 0;
}
...
void emxInitStruct_struct0_T(const emlrtStack *sp, struct0_T *pStruct,
                             const emlrtRTEInfo *srcLocation)
{
  emxInitStruct_rtString(&pStruct->solution_status);
  emxInit_real_T(sp, &pStruct->zhat_inf_dj, 1, srcLocation);
  emxInit_real_T(sp, &pStruct->what_inf_dj, 1, srcLocation);
  emxInit_real_T(sp, &pStruct->zhis, 3, srcLocation);
}
...
/* End of code generation (PIPG_orig_precond_Dexplicit_emxutil.c) */
