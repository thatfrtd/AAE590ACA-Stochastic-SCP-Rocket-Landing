function [A_k, B_k_plus, B_k_minus, S_k, d_k, Delta] = discretize_error_dynamics_FOH_RKV65_6DoFq(f, A, B, S, N, tspan, x_ref, u_ref, s_ref, N_sub)
    ...
    % Modified to use quaternion exponential map to propagate quaternions
    % better
    %#codegen
    ...
    x_ref(7:10, :) = x_ref(7:10, :) ./ vecnorm(x_ref(7:10, :));
    ...
    for k = 1:N_sub
        ...
        for i = 2 : stages
            ...
            for j = 1 : (i - 1)
                if a{i}(j) ~= 0
                    w_before = x_est(w_i, :);
                    x_est(not_q_i, :) = a{i}(j) * dt_sub .* xdot_ki(not_q_i, :, j) + x_est(not_q_i, :);
                    x_est(q_i, :) = q_first_order_integrate(x_est(q_i, :), a{i}(j) * dt_sub, w_before, x_est(w_i, :));
        ...
        for j = 1 : stages
            if b(j) ~= 0
                w_kp1_before = x_kp1(w_i, :);
                x_kp1(not_q_i, :) = x_kp1(not_q_i, :) + dt_sub * b(j) * xdot_ki(not_q_i, :, j);
                x_kp1(q_i, :) = q_first_order_integrate(x_kp1(q_i, :), dt_sub * b(j), w_kp1_before, x_kp1(w_i, :));
...
function [q_kp1] = q_first_order_integrate(q_k, delta_t, w_k, w_kp1)
    w_avg = (w_k + w_kp1) / 2;
    q_kp1 = q_mul_array(q_k, qexp(w_avg * delta_t) + delta_t ^ 2 / 24 * [cross(w_k, w_kp1, 1); zeros([1, size(q_k, 2)])]);
end
