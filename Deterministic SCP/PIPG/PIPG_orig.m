function [z_star, w_star, sol_info] = PIPG_orig(Q, q, H, h, D, lambda, sigma, rho, tol_abs, tol_rel, j_check, j_restart, j_max, z_ref, w_ref)
...
for j = 1 : j_max
    ...
    w_jp1 = v_j + beta_j * (H * z_j - h);

    %% Projected gradient step
    z_jp1 = D.project_onto_cones(1 : N, (z_j - alpha_j * (Q * z_j + q + H' * w_jp1)));
 
    aff_violation(j) = norm(H * z_jp1 - h);
    zhis(:, j) = z_j;

    %%
    v_jp1 = w_jp1 + beta_j * H * (z_jp1 - z_j);


    %% Extrapolate variables
    w_jp1 = (1 - rho) * w_j + rho * w_jp1;
    z_jp1 = (1 - rho) * z_j + rho * z_jp1;
    v_jp1 = (1 - rho) * v_j + rho * v_jp1;
