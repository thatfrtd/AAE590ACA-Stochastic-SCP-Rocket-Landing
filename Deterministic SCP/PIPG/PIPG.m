function [z_star, what_star, sol_info] = PIPG(qhat, Hhat, h, D, L, L_inv, lambda, sigma, rho, tol_abs, tol_rel, tol_infeas, j_check, j_max, z_ref, what_ref)
...
for j = 1 : j_max
    ...
    zhat_jp1 = L * D.project_onto_cones(1 : N, L_inv * (zeta_j - alpha * (lambda * zeta_j + qhat + Hhat' * eta_j)));
 
    aff_violation(j) = norm(Hhat * zhat_jp1 - h);
    zhis(:, j) = zhat_j;

    %% Proportional-integral feedback of affine equality constraint violation
    what_jp1 = eta_j + alpha * (Hhat * (2 * zhat_jp1 - zeta_j) - h);
