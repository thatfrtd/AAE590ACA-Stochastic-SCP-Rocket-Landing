function [CasADi_sol] = CasADi_solve_6DoF_mass_convexified(x_initial, x_final, x_guess, u_guess, vehicle, N, delta_t, glideslope_angle_max)
...
x = opti.variable(13, N);  % N x 6 matrix
% Controls: thrust (percent), thrust_angle (rad)
u = opti.variable(5, N - 1);

% Initial and final conditions
opti.subject_to(x(1:12, N) == x_final); % Final state
...
for i = 1:(N-1)
    ...
    x_next = rk4(@(t, x, u, p) SymDynamicsEuler6DoF_convex(x, u, vehicle.L, vehicle.I, vehicle.alpha), delta_t * (i - 1), x_current, @(t) u_current, 0, delta_t);
...
% Thrust bounds
opti.subject_to(u(4, :) >= vehicle.min_thrust * exp(-x(13)));
opti.subject_to(u(4, :) <= vehicle.max_thrust * exp(-x(13)));
