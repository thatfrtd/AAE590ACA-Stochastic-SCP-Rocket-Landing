...
%% Get Dynamics
%f = @(t, x, u, p) state_equation(x, u, 1);
f = @(t, x, u, p) dynamics_asteroidmining(t, x, u, 1);
...
%% Specify Objective
if u_hold == "ZOH"
    min_fuel_objective = @(x, u, p) sum(norms(u, 2, 1)) * tf / N;
elseif u_hold == "FOH"
...
problem = DeterministicProblem(x_0, x_f, N, u_hold, tf, f, guess, convex_constraints, min_fuel_objective, scale = scale, terminal_bc = terminal_bc, integration_tolerance = 1e-12);
...
scatter(x_f(1), x_f(2), DisplayName=sprintf("Mars at t = %.g", tf)); hold off;
