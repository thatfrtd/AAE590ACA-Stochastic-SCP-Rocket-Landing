...
if not load:
    ...
    loaded_data = np.load(r'C:\Users\thatf\OneDrive\Documents\Purdue Classes\AAE 590ACA\AAE590ACA-Applied-Control-in-Astronautics\HW6\Parts\Q1\saved_3DoF_input.npz', allow_pickle=True)
    ...
    # Saved Problem
    dbfile = open(r'C:\Users\thatf\OneDrive\Documents\Purdue Classes\AAE 590ACA\AAE590ACA-Applied-Control-in-Astronautics\HW6\Parts\Q1\det_3DoF_prob', 'rb')    
...
if save:
    np.savez("saved_3DoF_input.npz", A_k = A_k, B_k = B_k, c_k = c_k, w_vc = w_vc, w_tr = w_tr, delta_t = delta_t, N = N, gimbal_max_angle = gimbal_max_angle, glideslope_max_angle = glideslope_max_angle, alpha = alpha, T_min = T_min, T_max = T_max, x_0 = x_0, x_f = x_f, x_ref = x_ref, u_ref = u_ref)
    
    dbfile = open('det_3DoF_prob', 'ab')
...
if str(type(problem)) == "<class 'array.array'>":
    ...
    U = cp.Variable((nu, N + 0), name='U')
    eta = cp.Variable((1, N + 0), name = 'eta')
    ...
    u_ref_param = cp.Parameter((nu, N + 0), name = 'u_ref')

    # Define objective
    objective = cp.Minimize(cp.sum(U[2,:]) * delta_t)
    ...
    for k in range(N):
        ...
        dynamics_constraints.append(X[:, k + 1] == Ak_params[k] @ X[:, k] + Bk_params[k] @ U[:, k] + ck_param[:, k])
    ...
    constraints = dynamics_constraints + [ # Dynamics constraint
                   T_min * cp.multiply(np.exp(-z_lb_k), (1 - (X[z_i, 0:-1] - z_lb_k) + 0.5 * (X[z_i, 0:-1] - z_lb_k) ** 2)) <= U[sigma_i, :], # Thrust min constraint
                   U[sigma_i, :] <= T_max * cp.multiply(np.exp(-z_lb_k), (1 - (X[z_i, 0:-1] - z_lb_k))), # Thrust max constraint
                   ...
                   cp.norm(X[:, 0:N] - x_ref_param[:, 0:N], 2, 0) + cp.norm(U - u_ref_param[:, 0:N], 2, 0) <= eta]
...
for k in range(N):
    ...
    problem.param_dict["Bk_" + str(k)].value = B_k[:, :, k]
