...
vehicle = Vehicle(m_wet, L, L * 3, gimbal_max, T_min, T_max, I = I, alpha = alpha);
...
scale_hint.x_max = [max(r_0) * ones([2, 1]); max(v_0) * ones([2, 1]); pi * ones([1, 1]); max(w_0) * ones([1, 1]); m_0];
scale_hint.x_min = [-max(r_0) * ones([2, 1]); -max(v_0) * ones([2, 1]); -pi * ones([1, 1]); -max(w_0) * ones([1, 1]); m_dry];
scale_hint.u_max = [T_max; sin(gimbal_max) * ones([1, 1])];
scale_hint.u_min = [T_min; -sin(gimbal_max) * ones([1, 1])];
scale_hint.p_max = 80;
scale_hint.p_min = 40;
...
max_gimbal_constraint = {1:N, @(t, x, u, p) abs(u(2)) - gimbal_max};
...
guess.u = [guess.u(3, :) * 1.15; atan2(guess.u(2, :), guess.u(1, :))];

%% Construct Problem Object
prob_3DoF = DeterministicProblem(x_0, x_f, N, u_hold, tf, f, guess, convex_constraints, min_fuel_objective, scale = scale, terminal_bc = terminal_bc, discretization_method = "error", N_sub = 1, scale_hint = scale_hint);
...
figure
plot(t_k, guess_scaled.u)

guess_scaled.u = [guess_scaled.u(1, :) .* cos(guess_scaled.u(2, :)); guess_scaled.u(1, :) .* sin(guess_scaled.u(2, :)); guess_scaled.u(1, :)];
...
% 
% guess.x = ptr_sol.x(:, :, ptr_sol.converged_i);
% guess.u = ptr_sol.u(:, :, ptr_sol.converged_i);
...
%%
% ptr_sol_cvx_u = [ptr_sol_cvx.u(3, :) .* ptr_sol_cvx.x(7, :); atan2(ptr_sol_cvx.u(2, :), ptr_sol_cvx.u(1, :))];
% 
% [prob_3DoF, Delta_disc] = prob_3DoF.discretize(ptr_sol_cvx.x, ptr_sol_cvx_u, guess.p);
% %%
% x_disc = prob_3DoF.disc_prop(ptr_sol_cvx_u, guess.p);
% %%
% [t_cont, x_cont, u_cont] = prob_3DoF.cont_prop(ptr_sol_cvx_u, guess.p);
% u_cont = [u_cont(1, :) .* cos(u_cont(2, :)); u_cont(1, :) .* sin(u_cont(2, :)); vecnorm(u_cont)];
% %%
% figure
% comparison_plot_3DoF_trajectory({ptr_sol_cvx.x, x_cont, x_disc}, ["Guess", "Continuous Propagation", "Discrete Propagation"], glideslope_angle_max, linestyle = [":", "-", "--"], title = "Continuous vs Discrete Propagation of Initial Guess")

%%
figure
plot_3DoF_trajectory(t_cont, x_cont, u_cont, glideslope_angle_max, gimbal_max, T_min, T_max, step = 1)

% %%
% guess.x = ptr_sol_cvx.x;
% guess.u = ptr_sol_cvx_u;
...
%%
figure
plot_3DoF_trajectory(t_cont_sol, x_cont_sol, u_cont_sol_p, glideslope_angle_max, gimbal_max, T_min, T_max, step = 1)
...
for i = 1:ptr_sol.converged_i
    ...
    u_iters{i} = [ptr_sol.u(1, :, i) .* cos(ptr_sol.u(2, :, i)); ptr_sol.u(1, :, i) .* sin(ptr_sol.u(2, :, i)); ptr_sol.u(1, :, i)];
    linestyle(i) = "-";
end
figure
...
opt_time = t_k;
control_inputs = ptr_sol.u(:, :, i);%u_sol_p(1:2, :) ./ ptr_sol.x(7, :, i);
input_vector = 1:nu;
x_opt = ptr_sol.x(:, :, i);
x_opt(7, :) = log(x_opt(7,:));
state_vector = 1:nx;
