...
gimbal_STC_trigger_height = 100 * 1e-3; % [km]
...
%% Get Dynamics
f = @(t, x, u, p) SymDynamicsQuat6DoF_localrot_alphabeta(x, u, L, I, alpha, g) * p(1);
...
tf_min_constraint = {1, @(t, x, u, p) 25 - p(1)};

state_convex_constraints = {glideslope_constraint, mass_constraint, angular_velocity_constraint, tf_min_constraint};
...
control_nonconvex_constraints = {max_gimbal_constraint_linearized, max_gimbal_constraint_linearized_STC};
...
terminal_bc = @(x, p, x_ref, p_ref) [0; 0; x(3:13) - x_f(3:13); 0];
...
elseif u_hold == "FOH"
    min_fuel_objective = @(x, u, p, x_ref, u_ref, p_ref) -x(14, end) / m_0 * 0 + sum(norms(u(1:3, :), 2, 1)) * alpha * p_ref(1) / (N - 1) / m_0 + sum(norms(u_ref(1:3, :), 2, 1)) * alpha * p(1) / (N - 1) / m_0 + sum(abs(u_ref(4, :))) * p(1) / (N - 1) + sum(abs(u(4, :))) * p_ref(1) / (N - 1)  + 0 * sum(abs(x(11, :)));% + sum_square(u(4, :)); %sum((u(4, 1:(end - 1)) + u(4, 2:end)) / 2) * delta_t;
end
...
prob_6DoF = DeterministicProblem(x_0, x_f, N, u_hold, 1, f, guess, convex_constraints, min_fuel_objective, scale = scale, scale_hint = scale_hint, terminal_bc = terminal_bc, nonconvex_constraints = nonconvex_constraints, discretization_method = "error", N_sub = 1, Name = "Deterministic_6DoF_fixed");

prob_6DoF.params = [T_min, T_max, tau_max, alpha, glideslope_angle_max, gimbal_max, pitch_max, angvel_max, time_min_max_thrust, max_gimbal_rate, diag(prob_6DoF.scaling.S_x)', diag(prob_6DoF.scaling.S_u)', prob_6DoF.scaling.c_x', prob_6DoF.scaling.c_u'];
...
nexttile
plot(0:ptr_sol.converged_i, [prob_6DoF.objective(prob_6DoF.guess.x, prob_6DoF.guess.u, prob_6DoF.guess.p, prob_6DoF.guess.x, prob_6DoF.guess.u, prob_6DoF.guess.p), [ptr_sol.info.J]]); hold on
...
tf_best = ptr_sol.p(1, i);

[t_cont_sol, x_cont_sol, u_cont_sol] = prob_6DoF.cont_prop(ptr_sol.u(:, :, i), ptr_sol.p(:, i));

figure
plot_6DoFq_ab_trajectory(t_k * tf_best, ptr_sol.x(:, :, i), ptr_sol.u(:, :, i), glideslope_angle_max, gimbal_max, T_min, T_max, step = 1)
...
figure
comparison_plot_6DoFq_ab_time_histories({t_k * tf_best, t_cont_sol * tf_best, t_k * tf_best}, {guess.x, x_cont_sol, ptr_sol.x(:, :, i)}, {guess.u, u_cont_sol, ptr_sol.u(:, :, i)}, ["Guess", "Cont", "Disc"], linestyle = [":", "-", "--"], title = "Continuous vs Discrete Propagation of Solution")
...
figure
plot(t_cont_sol * tf_best, rad2deg(theta_1), DisplayName="Pitch"); hold on
plot(t_cont_sol * tf_best, rad2deg(theta_2), DisplayName="Yaw"); hold on
plot(t_cont_sol * tf_best, rad2deg(theta_3), DisplayName="Roll"); hold off
grid on
legend()
%%
figure
plot(t_cont_sol * tf_best, acosd(pagemtimes([0, 0, 1], quat_rot_array(x_cont_sol(7:10, :), repmat([1; 0; 0], 1, numel(t_cont_sol)))))); hold on
plot(t_cont_sol * tf_best, acosd(2 * (x_cont_sol(7, :) .* x_cont_sol(9, :) - x_cont_sol(8, :) .* x_cont_sol(10, :)))); hold off
%%
% figure
...
thrust_rate_check_disc = abs(u_sol(1, 2:end) - u_sol(1, 1:(end-1))) / (tf_best / (N - 1));

gimbal_rate_check = rad2deg((vecnorm(u_cont_sol(2:3, 2 : end) - u_cont_sol(2:3, 1 : (end - 1)))) ./ diff(t_cont_sol * tf_best)');
gimbal_rate_check_disc = rad2deg((vecnorm(u_sol(2:3, 2 : end) - u_sol(2:3, 1 : (end - 1)))) ./ (tf_best / (N - 1)));

figure
tiledlayout(1, 2)

nexttile
plot(t_cont_sol(2:end) * tf_best, gimbal_rate_check); hold on
stairs(t_k(1:(end - 1)) * tf_best, gimbal_rate_check_disc, LineStyle="--"); 
...
nexttile
stairs(t_k(1:(end - 1)) * tf_best, thrust_rate_check_disc);  hold on
