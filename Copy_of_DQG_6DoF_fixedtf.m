...
scale_hint.u_max = [T_max; delta_max; delta_max; tau_max * ones([3, 1])];
scale_hint.u_min = [T_min; -delta_max; -delta_max; -tau_max * ones([3, 1])];
scale_hint.p_max = 150;
scale_hint.p_min = 60;

%% Get Dynamics
f = @(t, x, u, p) SymDynamicsDualQuat6DoF_alphabeta(x, u, 1, g, vehicle.L, vehicle.I, alpha_ME, alpha_RCS);
...
alphabeta_constraint = {1:N, @(t, x, u, p) norm(u([alpha_ind, beta_ind]), 2) - delta_max};

max_tau_constraint = {1:N, @(t, x, u, p) norm(u(tau_ind), Inf) - tau_max};
control_convex_constraints = {max_thrust_constraint, min_thrust_constraint, alphabeta_constraint, max_tau_constraint};
...
min_delta_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) max(0, -deltadot_max * tf / (N - 1) + u_ref(alpha_ind, k - 1)) - u(alpha_ind)};
max_delta_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) u(alpha_ind) - min(delta_max, deltadot_max * tf / (N - 1) + u_ref(alpha_ind, k - 1))};
min_phi_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) max(0, -phidot_max * tf / (N - 1) + u_ref(beta_ind, k - 1)) - u(beta_ind)};
max_phi_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) u(beta_ind) - min(2 * pi, phidot_max * tf / (N - 1) + u_ref(beta_ind, k - 1))};
...
delta_guess = deg2rad(1e-3) * ones([1, N]) * 0 + 0.00;
phi_guess = linspace(0, pi, N) * 0 + 0.00;
