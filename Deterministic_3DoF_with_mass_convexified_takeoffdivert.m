...
m_wet = 1000; % [kg]
m_0 = m_dry + m_wet;
T_max = 2 * m_0 * 9.81e-3; % [kg km / s2]
T_min = 0.55 * m_0 * 9.81e-3; % [kg km / s2]
...
glideslope_constraint = {round(N / 2) + 8:N, @(t, x, u, p) norm(x(1:2)) - x(2) / cos(glideslope_angle_max)};
hop_constraint_plus = {round(N / 2) + (-8:8), @(t, x, u, p) x([1:2, 6]) - ([25e-3; 50e-3; 0] + 1e-5)};
hop_constraint_minus = {round(N / 2) + (-8:8), @(t, x, u, p) [25e-3; 50e-3; 0] - 1e-5 - x([1:2, 6])};
state_convex_constraints = {glideslope_constraint, hop_constraint_plus, hop_constraint_minus};
...
glideslope_STC_angle = deg2rad(2);
glideslope_STC_trigger_height = 0.02; % [km]
glideslope_STC_constraint = {round(N / 2):N, @(t, x, u, p, x_ref, u_ref, p_ref, k) 1e4 * (norm(x(1:2)) - x(2) / cos(glideslope_STC_angle)) * (x_ref(2, k) <= glideslope_STC_trigger_height), ...
                                         @(t, x, u, p, k) 1e4 * (norm(x(1:2)) - x(2) / cos(glideslope_STC_angle)) * (x(2) <= glideslope_STC_trigger_height)};
...
%% Create Guess
sl_guess_up = guess_3DoF(x_0(1:6), [25e-3; 50e-3; x_f(3:end)], round(N / 2), round(N / 2), delta_t, vehicle);
sl_guess_down = guess_3DoF([25e-3; 50e-3; x_f(3:end)], x_f + [0; 0; 0; 0; 0; 0], N - round(N / 2), N - round(N / 2), delta_t, vehicle);
sl_guess.x = [sl_guess_up.x, sl_guess_down.x];
sl_guess.u = [sl_guess_up.u, sl_guess_down.u];
if u_hold == "ZOH"
...
%%
%ptr_sol = sol_struct.opt_sol; %ptr_sol_vc;
ptr_sol = ptr_sol_vc;
...
figure
plot_3DoFc_trajectory(t_cont_sol, x_cont_sol, u_cont_sol, glideslope_angle_max, gimbal_max, T_min, T_max, step = 100)

figure
plot_3DoFc_time_histories(t_cont_sol, x_cont_sol, u_cont_sol)
...
%%
figure
comparison_plot_3DoF_trajectory({guess.x, x_cont_sol, ptr_sol.x(:, :, i)}, ["Guess", "Cont", "Disc"], glideslope_angle_max, linestyle = [":", "-", "-"], title = "", STC_glideslope_angle_height = [glideslope_STC_angle, glideslope_STC_trigger_height])
