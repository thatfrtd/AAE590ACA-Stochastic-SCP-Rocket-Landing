...
scale_hint.x_max = [m_i; 1 * ones([4, 1]); max(r_I_i) * ones([4, 1]); w_max * ones([3, 1]); v_max * ones([3, 1])];
scale_hint.x_min = [m_f; -1 * ones([4, 1]); -max(r_I_i) * ones([4, 1]); -w_max * ones([3, 1]); -v_max * ones([3, 1])];
scale_hint.u_max = [T_max; delta_max; 2 * pi; tau_max * ones([3, 1])];
scale_hint.u_min = [T_min; 0; 0; -tau_max * ones([3, 1])];
scale_hint.p_max = 150;
scale_hint.p_min = 60;

%% Get Dynamics
f = @(t, x, u, p) SymDynamicsDualQuat6DoF(x, u, 1, g, vehicle.L, vehicle.I, alpha_ME, alpha_RCS);
...
state_convex_constraints = {min_mass_constraint};
...
control_convex_constraints = {max_thrust_constraint, min_thrust_constraint, max_gimbal_constraint, min_gimbal_constraint, max_tau_constraint};
...
min_thrust_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) max(T_min, -Tdot_max * tf / (N - 1) + u_ref(T_ind, k - 1)) - u(T_ind)};
max_thrust_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) u(T_ind) - min(T_max, Tdot_max * tf / (N - 1) + u_ref(T_ind, k - 1))};
min_delta_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) max(0, -deltadot_max * tf / (N - 1) + u_ref(delta_ind, k - 1)) - u(delta_ind)};
max_delta_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) u(delta_ind) - min(delta_max, deltadot_max * tf / (N - 1) + u_ref(delta_ind, k - 1))};
min_phi_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) max(0, -phidot_max * tf / (N - 1) + u_ref(phi_ind, k - 1)) - u(phi_ind)};
max_phi_rate_constraint = {2:N, @(t, x, u, p, x_ref, u_ref, p_ref, k) u(phi_ind) - min(2 * pi, phidot_max * tf / (N - 1) + u_ref(phi_ind, k - 1))};
...
w_B = zeros(3, N);
v_I = interp1([0; tf], [v_I_i'; [0; 0; v_z_I_f]'], t_k)';
...
if u_hold == "ZOH"
    scl_guess.x = [interp1([0, tf], [m_i, m_f], t_k(1:(end - 1))); scl_guess.x];
elseif u_hold == "FOH"
    scl_guess.x = [interp1([0, tf], [m_i, m_f], t_k); scl_guess.x];
end
%%
T_guess = scl_guess.x(1, :) * g;
T_guess = clip(T_guess, T_min, T_max);
delta_guess = deg2rad(1e-3) * ones([1, N]);
phi_guess = linspace(0, pi, N);
tau_guess = 1e-5 * ones([3, N]);
scl_guess.u = [T_guess; delta_guess; phi_guess; tau_guess]; 

%%

%CasADi_sol = CasADi_solve_6DoFdqg(x_0, x_f, scl_guess.x, scl_guess.u, g, vehicle, N, delta_t, alpha_ME, alpha_RCS, tau_max);
...
prob_6DoF = DeterministicProblem(x_0, qq_f, N, u_hold, tf, f, guess, convex_constraints, min_fuel_objective, terminal_bc = terminal_bc, scale = scale, scale_hint = scale_hint, nonconvex_constraints = nonconvex_constraints, discretization_method = "error", integration_tolerance = 1e-8);
...
%%
figure
plot_6DoFdqg_trajectory(t_cont, x_cont, u_cont, glideslope_angle_max, delta_max, T_min, T_max, step = 10)
%%
figure
plot_6DoFdqg_time_histories(t_cont, x_cont, u_cont)
...
%%
figure
plot_6DoFdqg_trajectory(t_cont_sol, x_cont_sol, u_cont_sol, glideslope_angle_max, delta_max, T_min, T_max, step = 1)

%%
figure
plot_6DoFdqg_time_histories(t_k, ptr_sol.x(:, :, i), ptr_sol.u(:, :, i))

%%
figure
plot_6DoFdqg_time_histories(t_cont_sol, x_cont_sol, u_cont_sol)
...
figure
comparison_plot_6DoFdqg_trajectory(x_iters, "iter " + string(1:ptr_sol.converged_i), glideslope_angle_max, linestyle = linestyle, title = "Solution vs Iteration")

figure 
comparison_plot_6DoFdqg_time_histories(t_iters, x_iters, u_iters, "iter " + string(1:ptr_sol.converged_i), linestyle = linestyle, title = "Solution vs Iteration")
