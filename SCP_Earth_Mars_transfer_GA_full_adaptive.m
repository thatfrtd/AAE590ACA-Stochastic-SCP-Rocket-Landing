...
terminal_bc = @(t, x, u, p) [x(1:4) - x_f(sum(p)); 0];
...
%% Get Dynamics
f = @(t, x, u, p, k) state_equation(x, u, mu, alpha) * (sum(p(1:(phase_transition_k - 1))) * (k < phase_transition_k) + sum(p((phase_transition_k + 1):end)) * (k > phase_transition_k));
...
min_fuel_objective = @(x, u, p, x_ref, u_ref, p_ref) einsum(@(k) norm(u(1:2, k)) * p_ref(k) + norm(u_ref(1:2, k)) * (p(k) - p_ref(k)), [1:(venus_GA_transition_k - 1), (venus_GA_transition_k + 1) : Nu]);
...
guess.p = [phase_1_s * ones([1 : venus_GA_transition_k, 1]) /  venus_GA_transition_k; phase_2_s * ones((venus_GA_transition_k + 1) : (N - 1), 1) / (N - 2 - venus_GA_transition_k)];
