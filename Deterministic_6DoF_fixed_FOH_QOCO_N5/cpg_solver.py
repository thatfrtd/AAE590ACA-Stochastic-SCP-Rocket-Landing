...
from Deterministic_6DoF_fixed_FOH_QOCO_N5 import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    if updated_params is None:
        updated_params = ["x_ref", "u_ref", "Ak_0", "Bk_minus_0", "Bk_plus_0", "ck", "Ak_1", "Bk_minus_1", "Bk_plus_1", "Ak_2", "Bk_minus_2", "Bk_plus_2", "Ak_3", "Bk_minus_3", "Bk_plus_3", "x_0", "x_f"]
    ...
    prob.var_dict['X'].save_value(np.array(res.cpg_prim.X).reshape((14, 5), order='F'))
    prob.var_dict['U'].save_value(np.array(res.cpg_prim.U).reshape((4, 5), order='F'))
    prob.var_dict['V'].save_value(np.array(res.cpg_prim.V).reshape((14, 4), order='F'))
    ...
    prob.constraints[4].save_dual_value(np.array(res.cpg_dual.d4).reshape(5))
    prob.constraints[5].save_dual_value(np.array(res.cpg_dual.d5).reshape(5))
    prob.constraints[6].save_dual_value(np.array(res.cpg_dual.d6).reshape(5))
    prob.constraints[7].save_dual_value(np.array(res.cpg_dual.d7).reshape(5))
    prob.constraints[8].save_dual_value(np.array(res.cpg_dual.d8).reshape(5))
    ...
    prob.constraints[11].save_dual_value(np.array(res.cpg_dual.d11).reshape((14, 14), order='F'))
    prob.constraints[12].save_dual_value(np.array(res.cpg_dual.d12).reshape((13, 13), order='F'))
...
def cpg_gradient(prob, gradient_sol_primal=None, gradient_sol_dual=None):
    ...
    if gradient_sol_primal is not None and gradient_sol_dual is not None:
        ...
        gradient_sol.primal = [0] * 372
        gradient_sol.dual = [0] * 799
        use_sol = False
    ...
    prob.param_dict['x_ref'].gradient = np.array(pdelta.x_ref).reshape((14, 5), order='F')
    prob.param_dict['u_ref'].gradient = np.array(pdelta.u_ref).reshape((4, 5), order='F')
    ...
    prob.param_dict['ck'].gradient = np.array(pdelta.ck).reshape((14, 4), order='F')
