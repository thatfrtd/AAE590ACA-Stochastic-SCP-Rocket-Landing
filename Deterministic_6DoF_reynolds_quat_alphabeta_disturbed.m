...
% Define disturbances
d_T = 1.1; % multiplicative thrust magnitude disturbance
d_a = [0; 0.2; 0] * 1e-3; % additive inertial acceleration disturbance [km / s2]

%% Get Dynamics
f = @(t, x, u, p) SymDynamicsQuat6DoF_localrot_alphabeta_disturbed(x, u, L, I, alpha, g, [d_T; d_a]);
...
max_thrust_constraint = {1:N, @(t, x, u, p) d_T * u(1) - T_max};
min_thrust_constraint = {1:N, @(t, x, u, p) T_min - d_T * u(1)};
...
%guess.x = ptr_sol_1.x(:, 8 : end, 11);
%guess.u = ptr_sol_1.u(:, 8 : end, 11);
