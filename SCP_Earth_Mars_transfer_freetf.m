...
%% Get Dynamics
f = @(t, x, u, p, k) state_equation(x, u, mu, alpha) * p(1);
f = multidynamic_f(f, 1:N);
...
%% Specify Objective
min_fuel_objective = @(x, u, p, x_ref, u_ref, p_ref) p(1)%sum(norms(u(1:2, :), 2, 1)) * p_ref(1) / (N - 1) + sum(norms(u_ref(1:2, :), 2, 1)) * (p(1) - p_ref(1)) / (N - 1);
