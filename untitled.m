...
while i <= length(xr)
    ...
    [t, traj] = ode45(@(t, Y) EoMwithDiscreteMatrixG(t, Y , u, A, B, c, nx, G), [0,dt], Y0, options);
...
function [dc, u, A] = lt_ot_con_2d(t,c,mu)
    ...
    dc = [v; -mu*r/(a^3)]+B*u;
    A = [zeros(2), eye(2); (-mu/(a^3))*eye(2) zeros(2)];
    %dc = [v; -r/(a^(3/2))]+B*u;
