...
alpha = sym("delta", [1;1]); % angle of attack
beta = sym("delta", [1;1]); % sideslip
...
Tau_B = [T * sin(alpha); T * sin(beta); T * cos(alpha) * cos(beta); tau];
...
matlabFunction(x_dot,"File","Dynamics Models/6DoF/SymDynamicsDualQuat6DoF_alphabeta","Vars",vars);
