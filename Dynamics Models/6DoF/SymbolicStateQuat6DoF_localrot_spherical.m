...
thrust_mag = sym("thrust_mag", [1;1]); % thrust magnitude 
delta = sym("delta", [1;1]); % gimbal deflection angle
phi = sym("phi", [1;1]); % gimbal azimuth angle
tau = sym("tau", 1);

u = [thrust_mag; delta; phi; tau];

T = thrust_mag * [cos(delta); sin(delta) * sin(phi); -sin(delta) * cos(phi)];
...
matlabFunction(x_dot,"File","Dynamics Models/6DoF/SymDynamicsQuat6DoF_localrot_spherical","Vars",vars);
