...
T = thrust_mag * [sqrt(1 - sin(alpha) ^ 2 - sin(beta) ^ 2); sin(alpha); sin(beta)];
...
% mdot
m_dot = -alpha_T * thrust_mag - alpha_T * tau;

x_dot = [r_dot; v_dot; qdot; w_dot; m_dot];
%j_a = jacobian(x_dot, x);
%j_b = jacobian(x_dot, u);

vars = [{x}; {u}; {L; I; alpha_T; g}];

% Create equations of motion function for optimizer
matlabFunction(x_dot,"File","Dynamics Models/6DoF/SymDynamicsQuat6DoF_localrot_alphabeta","Vars",vars);
