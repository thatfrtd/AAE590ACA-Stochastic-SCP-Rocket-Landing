...
thrust_mag = sym("T", [1;1]); % thrust magnitude 
alpha = sym("alpha", [1;1]); % gimbal deflection angle
beta = sym("beta", [1;1]); % gimbal azimuth angle
tau = sym("tau", 1);

u = [thrust_mag; alpha; beta; tau];

thrust_mag_dist = d_T * thrust_mag;

T = thrust_mag_dist * [cos(alpha) * cos(beta); sin(alpha); sin(beta)];

% Disturbances
disturbances = [d_T; d_a];
% d_T is multiplicative thrust disturbance
% d_a is additive inertial acceleration (wind)
...
% mdot
m_dot = -alpha_T * thrust_mag_dist - alpha_T * tau;

x_dot = [r_dot; v_dot; qdot; w_dot; m_dot];
%j_a = jacobian(x_dot, x);
%j_b = jacobian(x_dot, u);

vars = [{x}; {u}; {L; I; alpha_T; g}; {disturbances}];

% Create equations of motion function for optimizer
matlabFunction(x_dot,"File","Dynamics Models/6DoF/SymDynamicsQuat6DoF_localrot_alphabeta_disturbed","Vars",vars);
