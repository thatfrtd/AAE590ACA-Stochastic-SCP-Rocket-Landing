function [A_k, B_k_plus, B_k_minus, E_k, c_k, G_k, x] = integrate_stochastic_discrete_FOH(x0, A, B, E, c, f, G, u, p, tspan, tolerances)
    ...
    % Simulate    
    [~, y] = ode45(@(t, y) STM_diff_eq_FOH(t, y, A, B, E, c, f, G, u, p, sigma_plus, sigma_minus, nx), tspan, y0, tolerances);
...
function ydot = STM_diff_eq_FOH(t, y, A, B, E, c, f, G, u, p, sigma_plus, sigma_minus, n)
    x = y(1:n);
    STM = reshape(y((n + 1) : (n * (n + 1))), n, n);

    xdot = f(t, x, u(t), p);
    A_kdot = A(t, x, u(t), p) * STM;
    B_k_plusdot = STM \ B(t, x, u(t), p) * sigma_plus(t);
    B_k_minusdot = STM \ B(t, x, u(t), p) * sigma_minus(t);
    E_kdot = STM \ E(t, x, u(t), p);
    c_kdot = STM \ c(t, x, u(t), p);
    G_kdot = STM\G(t, x, u, p) * (STM \ G(t, x, u, p))';


    ydot = [xdot; A_kdot(:); B_k_plusdot(:); B_k_minusdot(:); E_kdot(:); c_kdot, G_kdot(:)];
end