function [] = plot_2DoF_MC_time_histories(t_mean, x_mean, u_mean, t_fb, x_MC_fb, u_MC_fb, t_k, X_k, S_k, t_no_fb, x_MC_no_fb, T_max, T_min, use_legend)
...
for i = 1:m
    stairs(t_fb(1:size(u_MC_fb, 2)), vecnorm(u_MC_fb(1:2, :, i), 2, 1) .* exp(x_MC_fb(5, 1:size(u_MC_fb, 2))), Color = [192, 192, 192] / 256, HandleVisibility='off'); hold on
end
...
for i = 1:m
    stairs(t_fb(1:size(u_MC_fb, 2)), atan2d(u_MC_fb(1, :, i), u_MC_fb(2, :, i)), Color = [192, 192, 192] / 256, HandleVisibility='off'); hold on
end
...
gimbal_99p9bound_up = atan2d(u_mean_full(1,:) + u1_3sigbound_full .* sign(u_mean_full(1,:)), u_mean_full(2, :) - u2_3sigbound_full);
gimbal_99p9bound_dn = atan2d(u_mean_full(1,:) - u1_3sigbound_full .* sign(u_mean_full(1,:)), u_mean_full(2, :) + u2_3sigbound_full);
...
stairs(t_k, atan2d(u_mean_full(1, :), u_mean_full(2, :)), Color = "k",LineWidth=1, DisplayName="Nominal"); hold on
stairs(t_k, gimbal_99p9bound_up, Color = [100, 100, 100] / 256, LineStyle=":", LineWidth=1, DisplayName="99.9% Bound"); hold on
stairs(t_k, gimbal_99p9bound_dn, Color = [100, 100, 100] / 256, LineStyle=":", LineWidth=1, HandleVisibility='off'); hold off
