function [] = covariance_plot(data, P_pred, P_tar, labels, title)
    ...
    for j = 1:n % row
        if i - 1 < j
            ...
            if i == j
                ...
                hold on; plot(x,fx_theory,'b', LineStyle="--"); %plot computed theoretical PDF  

                std_tar = sqrt(P_tar(i, i));
                x = 2 * 3 * std_tar * (-1:0.01:1);
                fx_theory = pdf('Normal', x, 0, std_tar); %theoretical normal probability density
                hold on; plot(x,fx_theory,'g'); %plot computed theoretical PDF  

                set(gca,'YColor','none')

                xlim([-6 * std_data, 6 * std_data])
                ...
                plot_cov_ellipse(P_pred([i, j], [i, j]), Color = "r"); hold on
                plot_cov_ellipse(cov(data([i,j], :)'), Color = "b", LineStyle = "--"); hold on
                plot_cov_ellipse(P_tar([i, j], [i, j]), Color = "g"); hold off
            end
...
function [] = plot_cov_ellipse(P, options)
    arguments
        P
        options.Color = "r"
        options.LineStyle = "-"
    ...
    plot(ellipse_3sigma_data(1, :), ellipse_3sigma_data(2, :), Color = options.Color, LineStyle = options.LineStyle);
end