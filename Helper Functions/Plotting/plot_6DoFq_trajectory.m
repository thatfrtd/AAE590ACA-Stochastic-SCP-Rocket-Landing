function [] = plot_6DoFq_trajectory(t, x, u, glideslope_angle, gimbal_max, T_min, T_max, options)
arguments
    t
    x 
    u 
...
u_N = quat_rot_array(x(7:10, 1:Nu), u(1:3, :));
...
cmap = colormap;
u_color = interp1(linspace(T_min / T_max, 1, size(cmap, 1)), cmap, vecnorm(u(1:3, :)) / T_max, "linear", "extrap");

gimbal_angle = acos(u(1, :) ./ vecnorm(u(1:3, :)));
...
polarplot(gimbal_clock(i + 1), rad2deg(gimbal_angle(i + 1)), Color = u_color(i + 1, :), LineWidth=1, Marker="o", MarkerFaceColor=u_color(i + 1, :)); hold on
...
cb = colorbar(pax, "southoutside",TickLabels=round(linspace(T_min / T_max, 1, 6) * 100, 2));
xlabel(cb, "Thrust [%]")
