function [t_sub, x_array, xhat_array, Phat_array, u_array] = propagate_cont_feedback_kalman_filter_tk(x_0, Ptilde_0, p, f, G, A_ref, B_ref, c_ref, x_ref, u_ref, K, L_k, C_k, D_k, f_0, g_0, t_k, tspan, N_sub, w_k_func, v_k, tolerances)
%CONT_KALMAN_FILTER Continuous propagation of continuous-discrete Kalman
%filter
%   Note - control is constant between measurements

nx = numel(x_0(:, 1));
t_sub = zeros(1, N_sub * (numel(t_k) - 1) + 1);
for i = 1: numel(t_k) - 1
    t_sub(N_sub*(i-1)+1:N_sub*i) = linspace(t_k(i), t_k(i+1), N_sub);
end
