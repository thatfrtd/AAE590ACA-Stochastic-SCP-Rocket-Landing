...
scale_hint.u_max = [T_max; gimbal_max; pi; pi / 4];
scale_hint.u_min = [T_min; 0; -pi; -pi / 4];
scale_hint.p_max = 60;
scale_hint.p_min = 20;

%% Get Dynamics
f = @(t, x, u, p) SymDynamicsQuat6DoF_localrot_spherical(x, u, L, I, alpha, g);
...
max_gimbal_constraint = {1:N, @(t, x, u, p) u(2) - gimbal_max};
min_gimbal_constraint = {1:N, @(t, x, u, p) 0 - u(2)};
%lcvx_thrust_constraint = {1:N, @(t, x, u, p) norm(u(1:3)) - u(4)}; 
max_vane_angle_constraint = {1:N, @(t, x, u, p) abs(u(4)) - deg2rad(0.5)};
control_convex_constraints = {max_gimbal_constraint,max_thrust_constraint, max_vane_angle_constraint, min_gimbal_constraint};
