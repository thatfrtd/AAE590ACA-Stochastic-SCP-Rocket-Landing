function [x_sol, u_sol, p_sol, X_sol, S_sol, sol_info] = solve_stochastic_ptr_convex_subproblem(prob, ptr_ops, x_ref, u_ref, p_ref, X_k_ref, S_k_ref)
...
cvx_begin
    ...
    minimize( prob.objective(prob.unscale_x(X), prob.unscale_u(U), prob.unscale_p(p), X_C, S_k, prob.unscale_x(x_ref), prob.unscale_u(u_ref), prob.unscale_p(p_ref), X_k_ref, S_k_ref) ...
    ...
    subject to
        ...
        for k = 1:prob.Nu
            % Convex Constraints
            for cc = 1:prob.n.cvx
                prob.convex_constraints{cc}(prob.unscale_x(X(:, k)), prob.unscale_u(U(:, k)), prob.unscale_p(p), X_C(:, (tri(k - 1) + 1):tri(k)), S_k(:, (tri(k - 1) + 1):tri(k))) <= 0;
            end
            % Nonconvex Constraints
            for nc = 1:prob.n.ncvx
                prob.nonconvex_constraints{nc}(prob.unscale_x(X(:, k)), prob.unscale_u(U(:, k)), prob.unscale_p(p), X_C(:, (tri(k - 1) + 1):tri(k)), S_k(:, (tri(k - 1) + 1):tri(k)), prob.unscale_x(x_ref), prob.unscale_u(u_ref), prob.unscale_p(p_ref), X_k_ref, S_k_ref, k) ...
                    - v_prime(nc) <= 0;
            end
        end
        v_prime >= 0;
...
for k = 1:prob.Nu
    % Nonconvex Constraints
    for nc = 1:prob.n.ncvx
        nc_ck(nc, k) = prob.nonconvex_constraints{nc}(prob.unscale_x(X(:, k)), prob.unscale_u(U(:, k)), prob.unscale_p(p), X_C(:, (tri(k - 1) + 1):tri(k)), S_k(:, (tri(k - 1) + 1):tri(k)), prob.unscale_x(x_ref), prob.unscale_u(u_ref), prob.unscale_p(p_ref), X_k_ref, S_k_ref, k);
            %- v_prime(nc);
        nc_ck_noref(nc, k) = prob.nonconvex_constraints{nc}(prob.unscale_x(X(:, k)), prob.unscale_u(U(:, k)), prob.unscale_p(p), X_C(:, (tri(k - 1) + 1):tri(k)), S_k(:, (tri(k - 1) + 1):tri(k)), prob.unscale_x(X), prob.unscale_u(U), prob.unscale_p(p), X_C, S_k, k);
    end
...
sol_info.J = prob.objective(prob.unscale_x(X), prob.unscale_u(U), prob.unscale_p(p), X_C, S_k);
...
sol_info.dJ = 100 * (prob.objective(prob.unscale_x(X), prob.unscale_u(U), prob.unscale_p(p), X_C, S_k) - prob.objective(prob.unscale_x(x_ref), prob.unscale_u(u_ref), prob.unscale_p(p), X_k_ref, S_k_ref)) / prob.objective(prob.unscale_x(x_ref), prob.unscale_u(u_ref), prob.unscale_p(p), X_k_ref, S_k_ref);
