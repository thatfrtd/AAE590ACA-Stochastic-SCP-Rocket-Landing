...
v_0 = make_R2(-deg2rad(60)) * [0.356; 0]; % [km / s]
w_0 = deg2rad(0); % [rad / s]
...
min_time_constraint = @(t, x, u, p) 20 - p;
parameter_convex_constraints = {min_time_constraint};
...
%CasADi_sol = CasADi_solve_mass_convexified_freetf(x_0, sl_guess.x, sl_guess.u, sl_guess.p, vehicle, N, delta_t, glideslope_angle_max);%
...
prob_3DoF = DeterministicProblem(x_0, x_f, N, u_hold, 1, f, guess, convex_constraints, min_fuel_objective, scale = scale, terminal_bc = terminal_bc, integration_tolerance = 1e-8, discretization_method = "error", N_sub = 1);
...
%%
prob_3DoF = DeterministicProblem(x_0, x_f, N, u_hold, 1, f, guess, convex_constraints, min_fuel_objective, scale = scale, terminal_bc = terminal_bc, integration_tolerance = 1e-12, discretization_method = "error", N_sub = 1);
